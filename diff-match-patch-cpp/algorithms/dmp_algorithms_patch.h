...
#include "algorithms/dmp_algorithms_common.h"
#include "algorithms/dmp_algorithms_diff.h"
#include "algorithms/dmp_algorithms_match.h"
#include "types/dmp_patch.h"
#include "types/dmp_settings.h"
#include "utils/dmp_stringpool_base.h"
...
protected:
    inline static constexpr void patch_addContext(const settings_t& settings, patch_t& patch, string_view_t text) noexcept {
        ...
        while (text.indexOf(pattern) != text.lastIndexOf(pattern)
               && pattern.length() < settings.Match_MaxBits - settings.Patch_Margin - settings.Patch_Margin) {
            ...
            while (i < n) {
                ...
                switch (d.operation) {
                    ...
                    case Operation::EQUAL:
                        if (d.text.length() >= 2 * settings.Patch_Margin) {
            ...
            while (index < n && len > 0) {
                ...
                switch (aDiff.operation) {
                    case Operation::INSERT:
                        ...
                        l += str_insert(postpatch_text, l, char_count2, aDiff.text);
                        break;
                    case Operation::DELETE:
                        patch.length1 += aDiff.text.length();
                        patch.diffs.push_back(aDiff);
                        l -= str_remove(postpatch_text, l, char_count2, aDiff.text.length());
                        break;
                    case Operation::EQUAL:
                        if (aDiff.text.length() <= 2 * settings.Patch_Margin && patch.diffs.size() != 0 && aDiff != diffs.back()) {
                        ...
                        if (aDiff.text.length() >= 2 * settings.Patch_Margin) {
                            // Time for a new patch.
                            if (patch.diffs.size() != 0) {
...
public:
    inline static constexpr patch_result_t patch_apply(const settings_t& settings, const patches_t& _patches, string_pool_t& pool,
        ...
        for (auto& aPatch : patches) {
            ...
            size_t start_loc {};
            size_t end_loc = npos;
            if (text1.length() > settings.Match_MaxBits) {
                // patch_splitMax will only provide an oversized pattern
                // in the case of a monster delete.
                start_loc = dmp_match::match_main(settings, text, text1.substring(0, settings.Match_MaxBits), expected_loc);
                if (start_loc != npos) {
                    end_loc = dmp_match::match_main(settings, text, text1.substring(text1.length() - settings.Match_MaxBits),
                                                    expected_loc + text1.length() - settings.Match_MaxBits);
            ...
            if (start_loc == npos) {
                ...
                if (end_loc == npos) {
                    ...
                    text2 = text.substring(start_loc, min(end_loc + settings.Match_MaxBits, text.length()) - start_loc);
                }
                if (text1 == text2) {
    ...
    inline static constexpr void patch_splitMax(const settings_t& settings, patches_t& patches, string_pool_t& pool) noexcept {
        ...
        for (int x = 0; x < patches.size(); x++) {
            ...
            while (bpi < bps) {
                ...
                while (bpi < bps && patch.length1 < patch_size - settings.Patch_Margin) {
                    ...
                    if (diff_type == Operation::INSERT) {
                        ...
                        diff_text = diff_text.substring(size_t(0), min(diff_text.length(), patch_size - patch.length1 - settings.Patch_Margin));
                        patch.length1 += diff_text.length();
                        start1 += diff_text.length();
                ...
                if (dtext1.length() > settings.Patch_Margin) {
                    postcontext = dtext1.substring(0, settings.Patch_Margin);
                } else {
                    postcontext = dtext1;
                }
                ...
                if (!empty) {
                    patches.splice(++x, 0, std::move(patch));
                }
            }
        }
    }
