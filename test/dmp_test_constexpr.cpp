...
#include "dmp/traits/dmp_clock_traits_none.h"
...
#ifndef DONT_COMPILE_CONSTEXPR_TESTS

namespace {
using traits = dmp::all_traits<dmp::non_allocating_string_traits, dmp::null_clock_traits, dmp::nonallocating_container_traits>;


using dmp_t = dmp::tests::diff_match_patch_test<traits>;
...
using alphabet_hash_t = typename traits::container_traits::template alphabet_hash<char_t, size_t>;
...
#    define assertEquals(Comment, Value1, Value2)                                                                                                    \
        if (!(Value1 == Value2))                                                                                                                     \
        return false
#    define assertFalse(Comment, Value1)                                                                                                             \
        if (Value1)                                                                                                                                  \
        return false
#    define assertTrue(Comment, Value1)                                                                                                              \
        if (!(Value1))                                                                                                                               \
        return false
#    define assertGreater(Comment, Value1, Value2)                                                                                                   \
        if (!(Value1 > Value2))                                                                                                                      \
        return false
#    define assertLessEqual(Comment, Value1, Value2)                                                                                                 \
        if (!(Value1 <= Value2))                                                                                                                     \
        return false


inline static constexpr bool diff_commonPrefixTest() noexcept {
    dmp_t dmp;

    // Detect any common suffix.
    assertEquals("diff_commonPrefix: Null case.", 0, dmp::diff_match_patch_common<traits>::diff_commonPrefix(STR("abc"), STR("xyz")));
...
DEFINE_TEST(constexpr_string, diff, commonPrefixTest)


inline static constexpr bool diff_commonSuffixTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, commonSuffixTest)


inline static constexpr bool diff_commonOverlapTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, commonOverlapTest)


inline static constexpr bool diff_halfmatchTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, halfmatchTest)


inline static constexpr bool diff_linesToCharsTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, linesToCharsTest)


inline static constexpr bool diff_charsToLinesTest() noexcept {
    dmp_t         dmp;
    string_pool_t pool;
...
DEFINE_TEST(constexpr_string, diff, charsToLinesTest)


inline static constexpr bool diff_cleanupMergeTest() noexcept {
    dmp_t         dmp;
    string_pool_t pool;
    ...
    assertEquals("diff_cleanupMerge: Merge interweave.",
                 diffs_t().addAll(Diff(Operation::DELETE, STR("ac")), Diff(Operation::INSERT, STR("bd")), Diff(Operation::EQUAL, STR("ef"))), diffs);
    ...
    dmp.diff_cleanupMerge(diffs, pool);
    assertEquals("diff_cleanupMerge: Empty merge.", diffs_t().addAll(Diff(Operation::INSERT, STR("a")), Diff(Operation::EQUAL, STR("bc"))), diffs);

    diffs.clear();
    diffs.addAll(Diff(Operation::EQUAL, STR("")), Diff(Operation::INSERT, STR("a")), Diff(Operation::EQUAL, STR("b")));
    dmp.diff_cleanupMerge(diffs, pool);
    assertEquals("diff_cleanupMerge: Empty equality.", diffs_t().addAll(Diff(Operation::INSERT, STR("a")), Diff(Operation::EQUAL, STR("b"))), diffs);

    return true;
}
DEFINE_TEST(constexpr_string, diff, cleanupMergeTest)


inline static constexpr bool diff_cleanupSemanticLosslessTest() noexcept {
    dmp_t         dmp;
    string_pool_t pool;
...
DEFINE_TEST(constexpr_string, diff, cleanupSemanticLosslessTest)


inline static constexpr bool diff_cleanupSemanticTest() noexcept {
    dmp_t dmp;
    ...
    dmp.diff_cleanupSemantic(diffs);
    assertEquals("diff_cleanupSemantic: Simple elimination.", Diffs().addAll(Diff(Operation::DELETE, STR("abc")), Diff(Operation::INSERT, STR("b"))),
                 diffs);
    ...
    assertEquals("diff_cleanupSemantic: Overlap elimination.",
                 Diffs().addAll(Diff(Operation::DELETE, STR("abc")), Diff(Operation::EQUAL, STR("xxx")), Diff(Operation::INSERT, STR("def"))), diffs);
    ...
    assertEquals("diff_cleanupSemantic: Reverse overlap elimination.",
                 Diffs().addAll(Diff(Operation::INSERT, STR("def")), Diff(Operation::EQUAL, STR("xxx")), Diff(Operation::DELETE, STR("abc"))), diffs);
...
DEFINE_TEST(constexpr_string, diff, cleanupSemanticTest)


inline static constexpr bool diff_cleanupEfficiencyTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, cleanupEfficiencyTest)


inline static constexpr bool diff_prettyHtmlTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, prettyHtmlTest)


inline static constexpr bool diff_textTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, textTest)


inline static constexpr bool diff_deltaTest() noexcept {
    dmp_t         dmp;
    string_pool_t pool;
...
DEFINE_TEST(constexpr_string, diff, deltaTest)


inline static constexpr bool diff_xIndexTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, xIndexTest)


inline static constexpr bool diff_levenshteinTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, levenshteinTest)


inline static constexpr bool diff_bisectTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, diff, bisectTest)


inline static constexpr bool diff_mainTest() noexcept {
    dmp_t         dmp;
    string_pool_t pool;
    ...
    assertEquals("diff_main: Simple case #2.", diffs,
                 dmp.diff_main(STR("Apples are a fruit."), STR("Bananas are also fruit."), false,
                               false /*comparing pointers of different objects is not allowed in constexpr context*/));
    ...
    if constexpr (sizeof(char_t) <= 1) {
        ...
        char_t s1[] = { static_cast<char_t>(L'\u0680') };
        char_t s2[] = { static_cast<char_t>(L'\u0681'), 'x' };

        char_t c = 0;
        diffs.addAll(Diff(Operation::DELETE, STR("a")), Diff(Operation::INSERT, string_view_t(s1, dmp::utils::array_size(s1))),
                     Diff(Operation::EQUAL, STR("x")), Diff(Operation::DELETE, STR("\t")), Diff(Operation::INSERT, string_view_t { &c, 1 }));
        assertEquals("diff_main: Simple case #3.", diffs,
                     dmp.diff_main(STR("ax\t"), dmp.concat(pool, string_view_t(s2, dmp::utils::array_size(s2)), string_view_t { &c, 1 }), false));
    }
    ...
    assertEquals(
        "diff_main: Overlap #2.", diffs,
        dmp.diff_main(STR("abcy"), STR("xaxcxabc"), false, false /*comparing pointers of different objects is not allowed in constexpr context*/));
    ...
    diffs.addAll(Diff(Operation::DELETE, STR("ABCD")), Diff(Operation::EQUAL, STR("a")), Diff(Operation::DELETE, STR("=")),
                 ...
                 Diff(Operation::INSERT, STR("-")), Diff(Operation::EQUAL, STR("efghijklmnopqrs")), Diff(Operation::DELETE, STR("EFGHIJKLMNOefg")));
    assertEquals("diff_main: Overlap #3.", diffs, dmp.diff_main(STR("ABCDa=bcd=efghijklmnopqrsEFGHIJKLMNOefg"), STR("a-bcd-efghijklmnopqrs"), false));
    ...
    assertEquals("diff_main: Large equality.", diffs,
                 dmp.diff_main(STR("a [[Pennsylvania]] and [[New"), STR(" and [[Pennsylvania]]"), false,
                               false /*comparing pointers of different objects is not allowed in constexpr context*/));
...
#    ifndef DISABLE_VERY_LONG_STRING_TEST
    ...
    assertEquals("diff_main: Simple line-mode.",
                 dmp.diff_main(a, b, true, false /*comparing pointers of different objects is not allowed in constexpr context*/),
...
DEFINE_TEST(constexpr_string, diff, mainTest)


inline static constexpr bool match_alphabetTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, match, alphabetTest)


inline static constexpr bool match_bitapTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, match, bitapTest)


inline static constexpr bool match_mainTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, match, mainTest)


inline static constexpr bool patch_patchObjTest() noexcept {
    // Patch Object.
    Patch p;
    p.start1  = 20;
    p.start2  = 21;
...
DEFINE_TEST(constexpr_string, patch, patchObjTest)


inline static constexpr bool patch_fromTextTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, patch, fromTextTest)


inline static constexpr bool patch_toTextTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, patch, toTextTest)


inline static constexpr bool patch_addContextTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, patch, addContextTest)


inline static constexpr bool patch_makeTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, patch, makeTest)


inline static constexpr bool patch_splitMaxTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, patch, splitMaxTest)


inline static constexpr bool patch_addPaddingTest() noexcept {
    dmp_t dmp;
...
DEFINE_TEST(constexpr_string, patch, addPaddingTest)

inline static constexpr bool patch_applyTest() noexcept {
    dmp_t dmp;
    ...
    assertEquals("patch_apply: Big delete, big change 1.",
                 STR("xabc12345678901234567890---------------++++++++++---------------12345678901234567890y\tFalse\tTrue"), dmp.toString(results));
    ...
    assertEquals("patch_apply: Compensate for failed patch.", STR("ABCDEFGHIJKLMNOPQRSTUVWXYZ--------------------1234567YYYYYYYYYY890\tFalse\tTrue"),
...
DEFINE_TEST(constexpr_string, patch, applyTest)
